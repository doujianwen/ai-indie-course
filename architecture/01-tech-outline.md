...
| Phase 7 | 内容策划 · 运营增长 · 防跑偏 | 第 18-20 章 | 全员 + 外部约束 | 内容系统、SEO/GEO 数据闭环、纠错清单 |
...
## Phase 7 — 内容策划 · 运营增长 · 防跑偏机制（v1.2 范围升级新增）

> 对应 PRD v1.2 的 M8 / M9 / M10。bookconv 已有真实实践与数据，本章全部用真实资产教学，禁止编造。

### 第 18 章：内容策划系统——一页吃整簇
- **要演示的真实操作**：打开 bookconv 的 `src/data/content/*.ts` / `src/data/blog/*.ts` / `src/data/guides/*.ts` 三类单源 + `CONTENT_MAP` 注册表，展示 87 篇内容页（36 博客 + 21 指南 + 30 转换页）如何从数据文件自动派生列表 / sitemap / RSS；讲 `docs/content/一页吃整簇策略.md` 的「同意图关键词变体合并到最强词页面」方法论，以及它如何把 `mobi→epub` 多页分散（位置 69.7）收敛成单页权威。
- 讲：内容策划不是写文章，是「搭一个能自我繁殖的内容系统」；长尾词如何选、内链锚文本如何不堆砌（禁通用锚文本、185 条编辑内链真实案例）。

### 第 19 章：SEO/GEO 运营增长——从收录到被 AI 引用
- **要演示的真实操作**：走查 `public/llms.txt` 与 `robots.txt` 对 GPTBot / ClaudeBot / CCBot 的放行策略（GEO 已落地）；用 `scripts/seo-critic.mjs` 现场跑「博文注册 / llms.txt / 死链 / ES 键 / hreflang」门禁（退出码 1 即门禁）；打开 GSC 真实数据讲「287 展示 / 0 点击」的诊断路径（选择性披露，截图脱敏）。
- 讲：为什么 sitemap 必须从数据源派生、hreflang 绝不硬编码 `/en`、外链素材必须回流仓库；GEO（Generative Engine Optimization，被 AI 引用）与经典 SEO 的异同。

### 第 20 章：防跑偏——给独立开发者装一个「第三视角」
- **要演示的真实操作**：借旧版 `ai-independent-dev/correction-agent` 的「纯纠错型智能体」概念，落成本为课程内建的「防跑偏」机制：范围冻结清单（需求阶段就砍掉 80% 想法）、阶段验收关卡（每阶段一个可交付小胜利）、纠错清单（phase0 需求验证 / phase1 MVP 形态 / phase2 营销转化 / phase3 规模化）。用 6 年 PM 转独立开发「为收藏插件自研富文本编辑器耗 3 个月」的真实案例讲「为什么需要第三视角」。
- 讲：独立开发者最大的风险不是不会写代码，是「当局者迷、没有外部约束」；机制设计比意志力可靠。
