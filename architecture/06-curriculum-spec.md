...
> 版本：v1.2（Phase 3 产物，v1.2 范围升级已并入）
> 来源：`01-tech-outline.md`（20 章）+ PRD v1.2（M1–M10 模块）+ `openapi.yaml` + `05-demo-spec.md`
> 范围：课程 V1（self-serve，M1–M10 全含；陪跑 M14 defer v2；v1.2 已扩内容策划 + SEO/GEO 运营 + 防跑偏）
...
| M8 | 内容策划系统 | 第 18 章（内容策划 · 一页吃整簇）| 用 bookconv 87 页内容体系真实案例 |
| M9 | SEO/GEO 运营增长 | 第 19 章（SEO/GEO · 从收录到被 AI 引用）| 用 bookconv llms.txt/robots/GSC/GA4 真实数据 |
| M10 | 防跑偏 / 第三视角纠错机制 | 第 20 章（防跑偏 · 第三视角）| 借旧版 correction-agent 概念落地 |
...
### M8 · 第 18 章（内容策划）
- **18 内容策划系统：一页吃整簇**：Phase 7 / 视频+图文 / 13min。演示：开 bookconv 三类单源（`content`/`blog`/`guides`）+ `CONTENT_MAP` 注册表，展示 87 页如何自动派生列表/sitemap/RSS；讲「一页吃整簇」如何把分散多页收敛成权威页、内链锚文本如何不堆砌。`lesson-18` ← 锚点：内容注册表图、长尾词选择清单。

### M9 · 第 19 章（SEO/GEO 运营）
- **19 SEO/GEO 运营增长：从收录到被 AI 引用**：Phase 7 / 视频+图文 / 14min。演示：走查 `llms.txt`/`robots` GEO 放行；跑 `seo-critic.mjs` 门禁（博文注册/llms.txt/死链/ES 键/hreflang）；GSC 真实数据诊断（选择性披露 + 截图脱敏）。`lesson-19` ← 锚点：GEO 放行清单、GSC 诊断清单。

### M10 · 第 20 章（防跑偏）
- **20 防跑偏：给独立开发者装一个第三视角**：Phase 7 / 视频 / 12min。演示：借 correction-agent 概念落地范围冻结 + 阶段验收 + 纠错清单（phase0–3）；用真实跑偏案例讲「为什么需要外部约束」。`lesson-20` ← 锚点：防跑偏清单、阶段验收关卡模板。
...
- **包含**：M1–M10 全部（上表 00 + 0X + 01–20 + G1），约 23 节，视频实战为主、图文为辅。v1.2 范围升级：内容策划（M8）+ SEO/GEO 运营增长（M9）+ 防跑偏机制（M10）从 v2 提前至 v1 主线。
- **defer v2**：M11 付费闭环深度 / M12 生产可观测 Sentry / M13 BullMQ 队列工程 / M14 30 天陪跑 Sprint。
